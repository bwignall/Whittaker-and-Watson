...
t If this value of (p be substituted in the equation we obtain a result which is, effectively, that 
of §9-7. 
...
J It will be proved in § 11-51 that, if K (x, ij) = K y, x), the equation D (X) = has at least one 
root. 
...
are </> (.r) = cos (?i - 2r) .r, and (  (.r) = sin (% - 2?-) .r ; where r assumes all positive integral 
...
Let  (f>m (x) = Ci, m (f>i ( ) + C2, m < 2 ( ) + ••• + Cm-i <pm-i (i ) + Urn   ), 
...
and it can be verified l)y substitution J that this function actually is a solution. 
...
4. Shew that even periodic solutions (with period Stt) of the diflferential equation 
...
provided that this series converges uniformly, where X , 0  (x) are the characteristic 
numbers and functions of K  x, |) and 2 a 0  (x) is the expansion off(x). 
...
are 1, cos?n.r, sin mx, the corresponding characteristic numbers being 1, l/A'", Ijh'", where 
