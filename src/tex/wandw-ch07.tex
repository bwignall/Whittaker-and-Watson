...
\Section{7}{1}{A formula due to Darhoux*TODO.}
...
then $B_{n}$ is called the $n$th \emph{\Bernoulli an
  number}\footnote{TODO}. 
It is found that\footnote{TODO}
...
X Tables of the first sixty-two \Bernoulli an numbers have been given by
...
by \hardsubsectionref{4}{4}{4} corollary, differentiate both sides of this equation any
...
The \Bernoulli an polynomial* of order v is defined to be the
coefficient of
...
From this, by equating coefficients of P \hardsubsectionref{3}{7}{3}), we have
...
In the formula of Darboux \hardsectionref{7}{1}) write </>n(0 ' 4' )> where (f>n t)
is the ?ith \Bernoulli an polynomial.
...
sufficiently small; see\hardsubsectionref{7}{3}{1}. W. M. A. 9
...
\Subsection{7}{3}{1}{Teixeira's extended form of Biirmanns theorem.}
...
t The expansion is justified by\hardsectionref{4}{7}, since S -! - j- converges
...
can be evaluated by\hardsubsectionref{5}{2}{2} or by \hardsectionref{6}{1}.
...
£2, 42, G ... 2n-2)% taken ni at a time, shew that
...
point 2 = 0. \addexamplecitation{Teixeira.}
...
become, by\hardsubsectionref{5}{2}{2} and § 01,
...
has two roots, namely 9]l + A/(l + - )f i % l ~ /. /( " ' /l ' > d our
expansion
...
I y j < 1 (a - l) -i - I . \addexamplecitation{McClintock.}
...
integrand are the poles off z) and the point z = x, we have, by\hardsectionref{6}{1},
