...
22-21. The addition-theorems for en u and dn u. 
...
[It will be seen in § 22*302 that this detinition of A' is equivalent to the definition as 
 TT a  in §21-61.] 
...
f sn (u + 2K+ 2iK') = - sn u, ( sn (a + 4>K + UK') = sn u, 
...
Hence, writing ii in place of  Kzjir, we at once get from (A) 
(1 + A;') sn  u, k) cd  u, k) = sn  (1 + k') u, ki], 
since 4<Az/7r = 2Au/K =(1 + k')ii; 
...
side the range of this book, the reader is referred to Jacobi, Fundamenta JYova, to Klein, 
...
Example. By considering the transformation ro = r+l, shew, by the method of 
§ 22-42, that 

sn  k'u, k2)=k' sd  u, k), 

where  -0= ± ik/k', and the upper or lower sign is taken according as R t)<0 or R (r) > ; 
...
products for the Jacobian elliptic functions ; writing a =  KxJtt, we obviously 
...
If now we multiply out wP  w, x) Q (— w;, x) and substitute for w- in terms 
...
on solving these as equations in  j, S.,, we obviously get results of the form 
...
Example 1. Verify that, in the case of real integi'als, the following scheme gives 
all possible essentially diflerent arrangements of sign, and determine the appropriate 
...
2 en Uo en u  sn (?<2 - %) dn Mj + sn ( 2 - M3) sn (W3 - ?<i) sn (i<i - Mo) dn ?ii dn Mo dn % = 0, 
...
1 + dn 'iu \ l + dn tcj \ l - aj dn ?< +  2 dn  14 - ag dn  % + a  dn* uj 
...
sn  ui + U-2 + U3) = A/D, en  ui + U2 + ii ) = BjD, dn  ui + M2 + %) = C'/i), 
where 
