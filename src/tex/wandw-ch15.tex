...
Now suppose that the branch of  z +  z - l)'-* cos ( |" which has to be taken is such that 
...
Accoi'dingly, in futm e, when z is not a real number such that —\ < z< 1, 
...
Vtt r(n + |) L (2 sin 0)5 2 (2% + 3) (2sin0)  
...
(4) If y = a( F(   x) + aiFi (x) + a2F2  x) + ..., where Uo,  !, aa? ...  *'i'e real constants, 
then the mean value of -j~, in the interval from x= —h to x= +A is a . (Leaute.) 

50. If Fn (•* ) be defined as in the preceding example, shew that, when —h <x <h, 
